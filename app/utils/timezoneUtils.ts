...
export const formatTimestampWithSecondsIST = (dateString: string): string => {
  try {
    ...
    if (hasTimezone) {
      ...
      // Format time with seconds directly without using Date object methods
      ...
      displayHours = displayHours || 12; // 0 should be 12
      ...
      // Format using Date object methods for local time
      let hours = istDate.getHours();
      const minutes = istDate.getMinutes().toString().padStart(2, '0');
...
export const formatFullDateTimeIST = (dateString: string): string => {
  try {
    ...
    if (hasTimezone) {
      // Parse UTC date and extract components
      const utcDate = new Date(dateString);
      if (isNaN(utcDate.getTime())) {
        console.error('Invalid UTC date string:', dateString);
        return 'Invalid Date';
      ...
      // Format directly without using Date object methods
      ...
      // No timezone info, treat as local IST
      const cleanTimestamp = dateString.replace(' ', 'T');
      const istDate = new Date(cleanTimestamp);
      if (isNaN(istDate.getTime())) {
        console.error('Invalid IST date string:', dateString);
